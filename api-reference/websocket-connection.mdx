...
<Card title="WebSocket Connection" icon="plug">
  **URL:** `wss://your-server.com/voice-stream`
</Card>
...
sequenceDiagram
    ...
    Server->>Client: {"type": "savants_connected"}
...
<Steps>
  ...
  <Step title="Await Confirmation">
    Wait for `savants_connected` message from server
  </Step>
...
| `AUTHENTICATING` | JWT token sent, awaiting confirmation | Monitor for `vapi_connected` |
        ...
        return new Promise((resolve, reject) => {
          ...
          this.ws.onmessage = (event) => {
            if (typeof event.data === 'string') {
              ...
              if (message.type === 'savants_connected') {
                resolve();
    ...
    class VoiceWebSocket {
      ...
      void _handleMessage(dynamic message) {
        if (message is String) {
          ...
          if (data['type'] == 'savants_connected') {
            _onConnected();
